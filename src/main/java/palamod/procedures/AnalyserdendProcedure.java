		...
		for (int index0 = 0; index0 < (int) (8); index0++) {
			for (int index1 = 0; index1 < (int) (8); index1++) {
				for (int index2 = 0; index2 < (int) (316); index2++) {
					if ((world.getBlockState(new BlockPos(xloop, yloop, zloop))).is(BlockTags.create(new ResourceLocation("forge:ores")))
							|| aready_iden) {
						ores = ores + 1;
					} else if ((world.getBlockState(new BlockPos(xloop, yloop, zloop))).getBlock() == Blocks.BEDROCK || aready_iden) {
						bedrock = bedrock + 1;
					} else if ((world.getBlockState(new BlockPos(xloop, yloop, zloop))).getBlock() == Blocks.AIR
							|| (world.getBlockState(new BlockPos(xloop, yloop, zloop))).getBlock() == Blocks.CAVE_AIR
					...
					} else if ((world.getBlockState(new BlockPos(xloop, yloop, zloop))).is(BlockTags.create(new ResourceLocation("minecraft:anvil")))
							|| aready_iden) {
						anvil = anvil + 1;
					} else if ((world.getBlockState(new BlockPos(xloop, yloop, zloop))).is(BlockTags.create(new ResourceLocation("forge:planks")))
							|| aready_iden) {
						planks = planks + 1;
					} else if ((world.getBlockState(new BlockPos(xloop, yloop, zloop))).is(BlockTags.create(new ResourceLocation("minecraft:anvil")))
							|| aready_iden) {
						anvil = anvil + 1;
					} else if ((world.getBlockState(new BlockPos(xloop, yloop, zloop))).is(BlockTags.create(new ResourceLocation("minecraft:wool")))
							|| aready_iden) {
						wool = wool + 1;
					} else if ((world.getBlockState(new BlockPos(xloop, yloop, zloop))).is(BlockTags.create(new ResourceLocation("minecraft:logs")))
							|| aready_iden) {
						logs = logs + 1;
					} else if ((world.getBlockState(new BlockPos(xloop, yloop, zloop))).is(BlockTags.create(new ResourceLocation("minecraft:planks")))
							|| aready_iden) {
						planks = planks + 1;
					} else if ((world.getBlockState(new BlockPos(xloop, yloop, zloop)))
							.is(BlockTags.create(new ResourceLocation("forge:cobblestone"))) || aready_iden) {
						cobblestone = cobblestone + 1;
					} else if ((world.getBlockState(new BlockPos(xloop, yloop, zloop))).is(BlockTags.create(new ResourceLocation("forge:chests")))
							|| aready_iden) {
						chest = chest + 1;
